...
The composer draft is user-visible UI text; it reaches a model request only when the user sends it, like any other typed input.

## Known Limitations and Deferred Work

- **Whole-tree refetch only** — the drawer re-requests the full listing per open and per refresh; there is no incremental change feed.

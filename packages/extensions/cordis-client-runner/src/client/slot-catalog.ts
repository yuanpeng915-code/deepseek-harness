...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-file-tree InsertBridge id \'filetree-insert-bridge\'',
    ],
    replaceRisk: 'none',
  ...
  {
    key: 'conversation.session.body.utilities',
    kind: 'list',
    scope: 'session',
    summary: 'Right-aligned utility strip pinned between the header divider and the resident scrollport — the message area\'s top edge.',
    doc: 'Right-aligned utility strip pinned between the header divider and the\nresident scrollport — the message area\'s top edge. Entries anchor their\nown dropdown overlays; the strip collapses while unoccupied.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-notes NotesUtility id \'notes\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.session.body.utilities\', () => ctx.slots.register(\n      { name: \'conversation.session.body.utilities\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-file-tree FileTreeDrawer id \'filetree-drawer\'',
      'client-ui-model-board ModelBoardPanel id \'model-board-panel\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-model-board ModelBoardButton id \'model-board\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    kind: 'list',
    scope: 'root',
    summary: 'Extra header actions beside search / view options / add workspace (declared by the WorkspaceBrowser entry).',
    doc: 'Extra header actions beside search / view options / add workspace (declared by the WorkspaceBrowser entry).',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [
      'client-ui-file-tree FileTreeToggle id \'filetree-toggle\'',
    ],
    replaceRisk: 'none',

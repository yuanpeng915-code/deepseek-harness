...
flowchart LR
  ...
  pkg_notes["notes"]
  svc_notes["ctx.notes<br/>Session sticky notes"]
  pkg_e2b["e2b"]
...
| `ctx.notes` | `core` | [`notes`](../packages/notes/notes) | - | `apiproxy` | - | 从逐条 note/* 事件折叠用户拥有的每会话便签，服务面板 Remote，并拥有可选的 notes:context 注入节与一次性导入 steer。 |

...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/host/file-tree': { kind: 'none', reason: 'The service lists workspace directories for the browser; it registers no prompt, schema, or model-visible output.' },
  'packages/client/ui-file-tree': { kind: 'none', reason: 'The drawer renders the host file-tree listing and writes paths into the composer draft; it never reaches the agent or the session log.' },
  'packages/client/ui-model-board': { kind: 'none', reason: 'The board renders embedded pricing constants client-side; it never reaches the agent, the session log, or a model request.' },
  'packages/client/ui-notes': { kind: 'indirect', reason: 'The panel verbs route the notes Remote (put/delete/setInject/import); the host NoteService owns the model-visible notes:context section and the import steer.' },
